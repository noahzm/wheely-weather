...
- Chrome (headers, nav buttons, settings/location title pills) uses `GlassView` from `expo-glass-effect` over the flat theme background; on web, screens and the `Stack` run with transparent backgrounds so the page's flat background (`src/global.css`) shows through instead of an opaque per-screen fill.
