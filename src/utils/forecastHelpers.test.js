...
describe("getDayConditionReason", () => {
  it("prioritizes hazardous weather codes over condition", () => {
    ...
    expect(getDayConditionReason(day({ weatherCode: 73 }))).toBe(
      "Wintry roads",
    );
    expect(getDayConditionReason(day({ weatherCode: 65 }))).toBe(
      "Heavy rain risk",
    );
  });
  ...
  it.each([
    ...
    ["Hot afternoon", { condition: "poor", high: 91 }],
    ...
    ["Warm afternoon", { condition: "marginal", high: 86 }],
    ...
    [
      ...
      { condition: "good", windSpeed: 12, rainChance: 25 },
    ],
...
describe("getBestDaysBlurb", () => {
  ...
  const weekWith = (condByIndex) =>
    WEEK.map((date, i) =>
      day({ date, condition: condByIndex[i] ?? "marginal" }),
    );
